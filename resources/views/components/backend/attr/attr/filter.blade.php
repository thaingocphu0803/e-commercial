    ...
    <div class="filter">
        <div class="flex flex-middle flex-space-between">
            ...
            <div class="action flex gap-10">
                ...
                <div class="flex gap-10">
                    <select name="publish" class="form-control  select2">
                        <option value="0" selected>
                            {{ __('table.chooseObject', ['attribute' => __('table.attrStatus')]) }}
                        </option>
                ...
                <div class="flex gap-10">
                    <select name="attr_catalouge_id" class="form-control  select2">
                        <option value="0" selected>
                            {{ __('table.chooseObject', ['attribute' => __('table.parentSection')]) }}
                        </option>
                        @foreach ($listNode as $item)
                            <option value="{{ $item->id }}" @selected(request('attr_catalouge_id') == $item->id)>{{ $item->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            @can('modules', 'attr.create')
                <a href="{{ route('attr.create') }}" class="btn btn-danger">
                    <i class="fa fa-plus">
                        {{ __('table.addObject', ['attribute' => __('dashboard.attr')]) }}
                    </i>
                </a>
            @endcan
