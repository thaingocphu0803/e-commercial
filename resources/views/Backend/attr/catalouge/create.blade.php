<x-backend.dashboard.layout>
    ...
    @php
        ...
        $url = isset($attrCatalouge)
            ? route('attr.catalouge.update', $attrCatalouge->attr_catalouge_id)
            : route('attr.catalouge.store');

        $title = isset($attrCatalouge) ? __('custom.editObject', ['attribute' => __('custom.attrGroup')]) : __('custom.addObject', ['attribute' => __('custom.attrGroup')]);

        $publish = $attrCatalouge->publish ?? '';
        $follow = $attrCatalouge->follow ?? '';
    @endphp
...
<div class="row">
    <form action="{{ $url }}" method="POST" class="box" id="attr_catalouge_form">
        @csrf
        ...
        <div class="wrapper wrapper-content animated fadeInRight">
            <div class="row">
                <div class="col-lg-9">
                                    ...
                                    <div class="col-lg-12">
                                        <div class="form-row flex flex-col gap-10">
                                            ...
                                            <x-backend.dashboard.form.input
                                                ...
                                                rowLength="12"
                                                :value="$attrCatalouge->description ?? ''"
                                                tag='textarea'
                                            />
                    ...
                    <x-backend.dashboard.album :album="$attrCatalouge->album ?? '' "/>
                        ...
                        <div class="ibox-content">
                            <div class="seo-wrapper flex flex-col gap-10">
                                <x-backend.dashboard.form.seo
                                    ...
                                    :value="$attrCatalouge->meta_title ?? ''"
                                />
                                <x-backend.dashboard.form.seo
                                    :labelName="__('custom.keyword')"
                                    inputName='meta_keyword'
                                    :value="$attrCatalouge->meta_keyword ?? ''"
                                />
                                <x-backend.dashboard.form.seo
                                    :labelName="__('custom.description')"
                                    inputName='meta_description'
                                    tag='textarea'
                                    :value="$attrCatalouge->meta_description ?? ''"
                                    />
                                <x-backend.dashboard.form.seo
                                    :labelName="__('custom.canonical')"
                                    inputName='canonical'
                                    :must="true"
                                    :value="$attrCatalouge->canonical ?? ''"
                                />
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-lg-3">
                                ...
                                <div class="col-lg-12">
                                    <div class="row">
                                        <x-backend.dashboard.form.select
                                            ...
                                            :data="$languages"
                                            :must="true"
                                            :value="$attrCatalouge->language_id ?? ''"
                                        />
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="ibox">
                        <div class="ibox-title">
                            <h5>
                                {{__('custom.chooseObject', ['attribute' => __('custom.attrGroupImage')])}}
                            </h5>
                        </div>
                        <div class="ibox-content">
                            <div class="row">
                                <div class="col-lg-12">
                                    <x-backend.dashboard.form.upload
                                        rowLength="12"
                                        :labelName="__('custom.attrGroupImage')"
                                        :value="$attrCatalouge->image ?? '' "
                                    />
                                    <img
                                        class="col-lg-12 hidden"
                                        id="img_show"
                                        :alt="__('custom.attrGroupImage')"
                                        height="250px"
                                    >
                                </div>
                            </div>
                                ...
                                <div class="col-lg-12 flex flex-col gap-10">
                                    <div class="flex gap-10">
                                        <select name="publish" class="form-control  select2">
                                            <option disabled selected>
                                                {{__('custom.chooseObject', ['attribute' => __('custom.attrGroupStatus')])}}
                                            </option>
                                    ...
                                    <div class="flex gap-10">
                                        <select name="follow" class="form-control  select2">
                                            <option disabled selected>
                                                 {{__('custom.chooseObject', ['attribute' => __('custom.attrGroupVision')])}}
                                            </option>
