<x-backend.dashboard.layout>
    ...
    @php
        $url = isset($attr)
            ? route('attr.update', $attr->attr_id)
            : route('attr.store');

        $title = isset($attr) ? __('custom.editObject', ['attribute' => __('custom.attr')]) : __('custom.addObject', ['attribute' => __('custom.attr')]);
    ...
    <form action="{{ $url }}" method="POST" class="box" id="attr_form">
        @csrf
        ...
        <div class="wrapper wrapper-content animated fadeInRight">
            <div class="row">
                <div class="col-lg-9">
                    ...
                    <x-backend.dashboard.album :album="$attr->album ?? '' "/>
                        ...
                        <div class="ibox-content">
                            <div class="seo-wrapper flex flex-col gap-10">
                                ...
                                <x-backend.dashboard.form.seo
                                    ...
                                    :value="$attr->meta_keyword ?? ''"
                                />
                                <x-backend.dashboard.form.seo
                                    :labelName="__('custom.description')"
                                    inputName='meta_description'
                                    tag='textarea'
                                    :value="$attr->meta_description ?? ''"
                                    />
                                <x-backend.dashboard.form.seo
                                    :labelName="__('custom.canonical')"
                                    inputName='canonical'
                                    :must="true"
                                    :value="$attr->canonical ?? ''"
                                />
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-lg-3">
                        ...
                        <div class="ibox-content">
                            <div class="row flex flex-col gap-10">
                                <x-backend.dashboard.form.select
                                    ...
                                    name="attr_catalouge_id"
                                    rowLength="12"
                                    :data="$listNode"
                                    :must="true"
                                    :value="$attr->attr_catalouge_id ?? ''"
                                />
                                ...
                                <x-backend.dashboard.form.multiselect
                                    ...
                                    rowLength="12"
                                    :data="$listNode"
                                    :value="$attr->catalouges ?? []"
                                    :parent="$attr->attr_catalouge_id ?? ''"
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="ibox">
                        <div class="ibox-title">
                            <h5>
                                {{__('custom.chooseObject', ['attribute' =>  __('custom.attrImage')])}}
                            </h5>
                        </div>
                        <div class="ibox-content">
                            <div class="row">
                                <div class="col-lg-12">
                                    <x-backend.dashboard.form.upload
                                        rowLength="12"
                                        :labelName="__('custom.attrImage')"
                                        :value="$attr->image ?? '' "
                                    />
                                    <img
                                        class="col-lg-12 hidden"
                                        id="img_show"
                                        :alt="__('custom.attrImage')"
                                        height="250px"
                                    >
                                </div>
                            </div>
                                ...
                                <div class="col-lg-12 flex flex-col gap-10">
                                    <div class="flex gap-10">
                                        <select name="publish" class="form-control  select2">
                                            <option disabled selected>
                                                {{__('custom.chooseObject', ['attribute' => __('custom.attrStatus')])}}
                                            </option>
                                    ...
                                    <div class="flex gap-10">
                                        <select name="follow" class="form-control  select2">
                                            <option disabled selected>
                                                {{__('custom.chooseObject', ['attribute' => __('custom.attrVision')])}}
                                            </option>
