<form action="{{ route('customer.catalouge.index') }}">
    @csrf
    ...
    <div class="filter">
        <div class="flex flex-middle flex-space-between">
                ...
                <div class="flex gap-10">
                    <select name="publish" class="form-control  select2">
                        <option value="0" selected>
                            {{ __('custom.chooseObject', ['attribute' => __('custom.customerCatalougeStatus')]) }}
                        </option>
            ...
            <div class="flex flex-middle gap-10">
                {{-- @can('modules', 'customer.catalouge.permission')
                    <a href="{{ route('customer.catalouge.permission') }}" class="btn btn-warning">
                        <i class="fa fa-key">
                ...
                @can('modules', 'customer.catalouge.create')
                    <a href="{{ route('customer.catalouge.create') }}" class="btn btn-danger">
                        <i class="fa fa-plus">
                            {{ __('custom.addObject', ['attribute' => __('custom.customerCatalouge')]) }}
                        </i>
                    </a>
                @endcan
            </div>
