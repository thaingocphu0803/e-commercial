    ...
    <div class="filter">
        <div class="flex flex-middle flex-space-between">
            ...
            <div class="action flex gap-10">
                ...
                <div class="flex gap-10">
                    <select name="publish" class="form-control  select2">
                        <option value="0" selected>
                            {{ __('custom.chooseObject', ['attribute' => __('custom.customerStatus')]) }}
                        </option>
                ...
                <div class="flex gap-10">
                    <select name="customer_catalouge_id" class="form-control  select2">
                        <option value="0" selected>
                            {{ __('custom.chooseObject', ['attribute' => __('custom.customerCatalouge')]) }}
                        </option>
                        @foreach ($customerCatalouges as $customerCatalouge)
                            <option value="{{ $customerCatalouge->id }}" @selected(request('customer_catalouge_id') == $customerCatalouge->id)>
                                {{ $customerCatalouge->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            @can('modules', 'customer.create')
                <a href="{{ route('customer.create') }}" class="btn btn-danger">
                    <i class="fa fa-plus">
                        {{ __('custom.addObject', ['attribute' => __('custom.customer')]) }}
                    </i>
                </a>
            @endcan
