<x-backend.dashboard.layout>
    ...
    <x-backend.dashboard.breadcrumb
        :title="__('custom.delObject', ['attribute'=>__('custom.source')])"
    />
            ...
            <div class="row">
                <div class="col-lg-5">
                    <h3 class="panel-title">
                        {{__('custom.ObjectInfor', ['attribute' => __('custom.source')])}}
                    </h3>
                    <div class="pannel-description">
                        {{__('custom.DelQuestion', ['attribute' => __('custom.source')])}}
                        <span class="text-danger">{{ $source->name }}</span>
                    </div>
                    <div class="pannel-description">
                        {{__('custom.DelNote')}}
                    </div>
                        ...
                        <div class="ibox-content">
                            <div class="row">
                                <x-backend.dashboard.form.input inputName="name" type="text" :labelName="__('custom.name')"
                                    :value="$source->name ?? ''" :readOnly="true" />

                                <x-backend.dashboard.form.input inputName="canonical" type="text"
                                    :labelName="__('custom.keyword')" :value="$source->keyword   ?? ''" :readOnly="true" />
